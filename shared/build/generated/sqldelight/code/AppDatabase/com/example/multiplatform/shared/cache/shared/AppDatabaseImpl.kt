package com.example.multiplatform.shared.cache.shared

import com.example.multiplatform.shared.cache.AppDatabase
import com.example.multiplatform.shared.cache.AppDatabaseQueries
import com.example.multiplatform.shared.cache.Rocket
import com.example.multiplatform.shared.cache.SelectAllLaunchesInfo
...
private class AppDatabaseQueriesImpl(
  ...
  override fun <T : Any> selectAllLaunchesInfo(mapper: (
    ...
    name: String?,
    type: String?
  ) -> T): Query<T> = Query(-1723495432, selectAllLaunchesInfo, driver, "AppDatabase.sq",
  ...
  override fun insertLaunch(
    ...
    notifyQueries(-2013047064, {database.appDatabaseQueries.selectAllLaunchesInfo})
  }
  ...
  override fun insertRocket(
    ...
    notifyQueries(-1828881911, {database.appDatabaseQueries.selectRocketById +
        database.appDatabaseQueries.selectAllLaunchesInfo})
  }

  override fun removeAllLaunches() {
    driver.execute(-30732382, """DELETE FROM Launch""", 0)
    notifyQueries(-30732382, {database.appDatabaseQueries.selectAllLaunchesInfo})
  }

  override fun removeAllRockets() {
    driver.execute(1274613758, """DELETE FROM Rocket""", 0)
    notifyQueries(1274613758, {database.appDatabaseQueries.selectRocketById +
        database.appDatabaseQueries.selectAllLaunchesInfo})
  }
  ...
  private inner class SelectRocketByIdQuery<out T : Any>(
    ...
    override fun execute(): SqlCursor = driver.executeQuery(-1965959170, """
    |SELECT * FROM Rocket
    |WHERE id = ?
    """.trimMargin(), 1) {
      bindString(1, id)
